...
class SwraidRebuildLVM(Test):
        ...
        if self.fs_name in fs_packages:
            ...
            if not smm.check_installed(fs_pkg):
                ...
                if not smm.install(fs_pkg):
                    self.log.warning("Unable to install %s, trying fallback", fs_pkg)
                    # Fallback to ext4 if preferred filesystem tools can't be installed
                    if self.fs_name != 'ext4':
                        self.fs_name = 'ext4'
                        if not smm.check_installed('e2fsprogs'):
                            if not smm.install('e2fsprogs'):
                                self.cancel("Unable to install any filesystem utilities")
    ...
    def wait_for_sync(self, timeout=600):
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if ("State : clean" in output or
                        ("State : active" in output and "resync" not in output.lower() and
                         "recovery" not in output.lower())):
                    elapsed = time.time() - start_time
                    self.log.info("RAID synchronization completed in %.2f seconds", elapsed)
                    return True
        ...
        elapsed = time.time() - start_time
        self.log.error("RAID sync did not complete within %d seconds (elapsed: %.2f)",
                       timeout, elapsed)
        return False
    ...
    def raid_creation_and_sync(self):
        ...
        self.log.info("Creating RAID%s array: %s", self.raidlevel, self.raid_name)
    ...
    def raid_rebuild_and_sync(self):
        ...
        if not self.rebuild_disk:
            self.log.info("Skipping rebuild test (not applicable for RAID level %s)",
                          self.raidlevel)
            return
        ...
        self.log.info("Simulating disk failure by removing: %s", self.rebuild_disk)
        ...
        self.log.info("Re-adding disk to trigger rebuild: %s", self.rebuild_disk)
        if not self.sraid.add_disk(self.rebuild_disk):
            self.fail("Failed to add disk %s" % self.rebuild_disk)

        self.log.info("Disk %s added successfully, rebuild started", self.rebuild_disk)
    ...
    def lvm_setup_on_raid(self):
        ...
        self.log.info("Creating Logical Volume %s (50%% of VG)...", self.lv_name)
        process.run("lvcreate -y -l 50%%VG -n %s %s" % (self.lv_name, self.vg_name),
                    shell=True)
